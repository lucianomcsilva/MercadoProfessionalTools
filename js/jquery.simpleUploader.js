// Inspired by timothypoon http://timothypoon.com/
...
function uploader(input, options)
{
	...
	//this.input = document.getElementById("fileBackground");
   //-------------------------------------------------------------------------------------------------------------------
   ...
   //-------------------------------------------------------------------------------------------------------------------
	...
	this.xhr.onreadystatechange = function(ev)
	{
		...
		{
			console.log('done!'+$this.xhr.responseText);
			if($this.xhr.status == 200) 
			{
            response = $this.xhr.responseText;
				if($this.settings.success) $this.settings.success(response, ev);
				$this.input.value = '';
			}
			else 
			{
